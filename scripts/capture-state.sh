...
#
# Usage: scripts/capture-state.sh        (from anywhere inside the repository)
...
R = "<REDACTED>"
RULES = [
    (re.compile(r"-----BEGIN [A-Z ]*PRIVAT[E] KEY-----.*?-----END [A-Z ]*PRIVAT[E] KEY-----"), "<REDACTED-PEM>"),
...
PEM_BEGIN = re.compile(r"-----BEGIN [A-Z ]*PRIVAT[E] KEY-----")
PEM_END = re.compile(r"-----END [A-Z ]*PRIVAT[E] KEY-----")
in_pem = False
...
run() {
  local disp=("$@") rc
  case ${disp[0]} in
    k) disp[0]=kubectl ;; h) disp[0]=helm ;; g) disp[0]=git ;;
...
LEAK_RE='do''p_v1_|gs''k_|gh''p_|github''_pat_|sk-''ant-|AK''IA|PRIVATE'' KEY'
leaks=$(grep -rlE -- "$LEAK_RE" "$OUT" 2>/dev/null)
if [[ -n $leaks ]]; then
  { echo "# leak self-check FAILED: a credential pattern matched in these files (matches not shown)"; echo "$leaks"; } > "$OUT/LEAK-CHECK.txt"
  echo "capture-state: leak self-check FAILED, see $OUT/LEAK-CHECK.txt" >&2
  exit 3
fi
echo "# leak self-check: no credential pattern found in $(find "$OUT" -type f | wc -l) files" > "$OUT/LEAK-CHECK.txt"
